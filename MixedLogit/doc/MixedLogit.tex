...
In expectation, the second-degree derivatives simplify:
\begin{equation}
\begin{aligned}
...
Alternatively, we can also estimate the Fisher information matrix using the empirical variance of the score function:
\begin{equation}
\begin{aligned}
	& \hat{I}_{aa'} && = \sum_m \sum_j y_{mj} M^{(1)}_{mja} M^{(1)}_{mja'} \\
	& \hat{I}_{al} && = \sum_m \sum_j y_{mj} M^{(1)}_{mja} M^{(2)}_{mjl} \\
	& \hat{I}_{ll'} && = \sum_m \sum_j y_{mj} M^{(2)}_{mjl} M^{(2)}_{mjl'} \\
\end{aligned}
\end{equation}
